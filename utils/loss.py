...
def RPN_loss(results, lambda_reg=1.0, total_samples=256, max_pos=128):
    ...
    for _, cls_p, box_p, label_p, shift_gt in results:
        ...
        all_labels.append(label_p)

        if pos_mask.any():
            all_reg_preds.append(box_p[pos_mask])
            all_reg_targets.append(shift_gt[pos_mask])

    # 拼接所有批次的结果
    all_cls_preds = jt.concat(all_cls_preds, dim=0)  # [N, 2]
    all_labels = jt.concat(all_labels, dim=0)        # [N]

    # 找出正负样本索引
    pos_inds = jt.nonzero(all_labels == 1).squeeze(-1)
    neg_inds = jt.nonzero(all_labels == 0).squeeze(-1)
    ...
    # 分类 loss，使用 CrossEntropyLoss 输入是 logits 和类别索引
    sample_preds = all_cls_preds[sample_inds]       # [采样数, 2]
    sample_labels = all_labels[sample_inds].long()  # [采样数]

    if jt.isnan(sample_preds).any() or jt.isnan(sample_labels).any():
        print("⚠️ 分类 loss 输入含 NaN！")

    cls_loss_fn = nn.CrossEntropyLoss()
    cls_loss = cls_loss_fn(sample_preds, sample_labels)

    # 回归 loss：只用正样本
    if num_pos > 0 and len(all_reg_preds) > 0:
        all_reg_preds = jt.concat(all_reg_preds, dim=0)
        all_reg_targets = jt.concat(all_reg_targets, dim=0)

        num_reg_pos = min(all_reg_preds.shape[0], num_pos)
        reg_sample_inds = jt.randperm(all_reg_preds.shape[0])[:num_reg_pos]

        reg_sample_pred = all_reg_preds[reg_sample_inds]
        reg_sample_target = all_reg_targets[reg_sample_inds]

        if jt.isnan(reg_sample_pred).any() or jt.isnan(reg_sample_target).any():
            print("⚠️ 回归 loss 输入含 NaN！")
    ...
    print(f"[RPN] total_loss: {total_loss.item():.4f}, cls_loss: {cls_loss.item():.4f}, reg_loss: {reg_loss.item():.4f}")
    print(f"[RPN] 正样本: {pos_inds.shape[0]}, 负样本: {neg_inds.shape[0]}, 采样正: {num_pos}, 采样负: {num_neg}, 总样本: {all_labels.shape[0]}")

    return total_loss, cls_loss, lambda_reg * reg_loss
'''
...
def RPN_loss(results, lambda_reg=1.0, total_samples=256, max_pos=128):
    ...
    for img_idx, (anchor, cls_p, box_p, label_p, shift_gt) in enumerate(results):
        ...
        if jt.isnan(sample_preds).any() or jt.isnan(sample_labels).any():
            print(f"⚠️ 图片 {img_idx} 分类 loss 输入含 NaN！")
        ...
        if num_pos > 0:
            ...
            if jt.isnan(reg_sample_pred).any() or jt.isnan(reg_sample_target).any():
                print(f"⚠️ 图片 {img_idx} 回归 loss 输入含 NaN！")
       ...
       # print(f"[RPN-Img {img_idx}] cls_loss: {cls_loss.item():.4f}, reg_loss: {reg_loss.item():.4f}, 正样本: {pos_inds.shape[0]}, 负样本: {neg_inds.shape[0]}")
    ...
    #print(f"[RPN-Batch] total_loss: {total_loss.item():.4f}, cls_loss: {avg_cls_loss.item():.4f}, reg_loss: {avg_reg_loss.item():.4f}")
...
def RPN_loss_sample(results, lambda_reg=1.0, total_samples=64, max_pos=32):
    ...
    for img_idx, (anchor, cls_p, box_p, label_p, shift_gt) in enumerate(results):
        ...
        # 采样正负样本索引
        pos_sample_inds = (
            pos_inds[jt.randperm(pos_inds.shape[0])[:num_pos]] if num_pos > 0 else jt.array([], dtype=jt.int64)
        )
        neg_sample_inds = (
            neg_inds[jt.randperm(neg_inds.shape[0])[:num_neg]] if num_neg > 0 else jt.array([], dtype=jt.int64)
        )
...
#    print(f'Rpn loss {total_loss} cls: {cls_loss}, box:{reg_loss}')
