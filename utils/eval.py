...
def detect_image(net, image_path, class_name, crop=True, count=False, nms_iou=0.3, confidence=0.8, num_classes=21):
    ...
    image_data =np.expand_dims(np.transpose(preprocess_input(np.array(image_data, dtype='float32')), (2, 0, 1)), 0)  #(1, C, H, W)
    ...
    for i, c in enumerate(top_label):
        predicted_class=class_name[int(c)]#绘制检测框和标签
        box=top_boxes[i]
        score=top_conf[i]
